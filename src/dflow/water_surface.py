...
def plot(t):
    ...
    wdt = griddata((nx, ny),
                   wd.isel(time=t).values, (triang.x, triang.y),
                   method='linear')

    tcf = ax.tricontourf(triang,
                         wdt,
                         levels,
...
vmax = np.array([np.ceil(abs(res.mesh2d_s1.values.max()))
...
for res, inp in zip(res_list, inp_list):
    ...
    utils.animation(plot, range(0, wd.time.shape[0], 1),
                    inp['label'].replace(' ', ''))
    if itr > 0:
