...
center = np.where((nx > inp_list[0]['x_center'] - 200) & (nx < inp_list[0]['x_center']))
ny_center = ny[center]
wd_center = [res.mesh2d_s1.values[:,center] for res in res_list]

ymax = np.array([np.ceil(abs(res.mesh2d_s1.values.max())) for res in res_list]).max()
ymin = -ymax

labels = [inp['label'] for inp in inp_list]

utils.animation(plot, range(0, res_list[0].time.shape[0], 1), inp_list[-1]['label'].replace(' ', ''))
